...
| await | Used to run a coroutine once an asynchronous event loop has already started running <br> &#8594; await can only be used inside a coroutine <br> &#8594; Coroutines must be called with ```await```, otherwise there will be a <code> RuntimeWarning </code> about enabling <code>tracemalloc</code>.|
